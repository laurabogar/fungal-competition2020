...
allocratios$pctNratio = rep(NA, nrow(allocratios))
    ...
    if (allocratios$Plant[i] == allocratios$Plant[j] &
      ...
      if (allocratios$compartment_fungus[i] == "Tt") {
        if (allocratios$compartment_fungus[j] != "None") {
          ...
          allocratios$pctNratio[i] = allocratios$uncolonized_roots.pctN[i]/allocratios$uncolonized_roots.pctN[j]
          allocratios$pctNratio[j] = NA # to ensure one entry per plant
...
allocratios$logpctNratio = log(allocratios$pctNratio)
...
#### Looking at pct N allocation ####

# I thought that the difference in C allocation
# could closely track differences in N concentrations
# in uncolonized roots, i.e. that the plant would
# put more C towards the side of the root system
# that had relatively higher tissue N concentrations. 
# I appear to have been wrong.
# This plot doesn't look much like the other one.

labels = c(High = "High N", Low = "Low N")

ggplot(data = allocratios) +
  geom_boxplot(outlier.alpha = 0,
               aes(x = versus, y = logpctNratio)) +
  geom_jitter(aes(x = versus, y = logpctNratio),
              width = 0.25) +
  geom_abline(intercept = 0, slope = 0, linetype = "dashed") +
  facet_grid(. ~ N_level, labeller = labeller(N_level = labels)) +
  ylab("Log N contents ratio relative to Tt") +
  theme(plot.margin = unit(c(1,1,1,1), "cm")) +
  xlab("Competitor") +
  scale_x_discrete(labels = c("None" = "None", 
                              "Sp" = "Other",
